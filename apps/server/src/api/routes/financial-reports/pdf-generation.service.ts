...
export class PdfGenerationService {
  ...
  includeApprovalMetadata(doc: PDFKit.PDFDocument, report: FinancialReport): void {
    ...
    if (report.dafId && report.dafApprovedAt) {
      doc.font('Helvetica-Bold').text('DAF Approval:', { underline: true });
      doc.moveDown(0.2);
      doc.font('Helvetica').text(`  Approved At: ${this.formatDate(report.dafApprovedAt)}`);
    ...
    if (report.dgId && report.dgApprovedAt) {
      ...
      doc.moveDown(0.2);
      doc.font('Helvetica').text(`  Approved At: ${this.formatDate(report.dgApprovedAt)}`);
      if (report.dgComment) {
