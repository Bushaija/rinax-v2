...
export const generateStatement: AppRouteHandler<GenerateStatementRoute> = async (c) => {
  ...
  try {
    ...
    for (const templateLine of template.lines) {
      ...
      // Task 4.2: Inject working capital balances for CHANGES_RECEIVABLES and CHANGES_PAYABLES
      // Note: We show the current period balance, not the cash flow adjustment
      if (statementCode === 'CASH_FLOW' && workingCapitalResult) {
        if (templateLine.lineCode === 'CHANGES_RECEIVABLES') {
          currentPeriodValue = workingCapitalResult.receivablesChange.currentPeriodBalance;
          previousPeriodValue = workingCapitalResult.receivablesChange.previousPeriodBalance;
          isWorkingCapitalComputed = true;
        } else if (templateLine.lineCode === 'CHANGES_PAYABLES') {
          currentPeriodValue = workingCapitalResult.payablesChange.currentPeriodBalance;
          previousPeriodValue = workingCapitalResult.payablesChange.previousPeriodBalance;
          isWorkingCapitalComputed = true;
        }
      }
...
export const exportStatement: AppRouteHandler<typeof import('./financial-reports.routes').exportStatement> = async (c) => {
  try {
    ...
    // Step 1: Generate the statement using the same logic as generateStatement
    // We create a mock context that returns the request body without export-specific fields
    ...
    // Type assertion needed because mockContext doesn't perfectly match the Hono context type
    const generateResponse = await (generateStatement as any)(mockContext);

    // The response from generateStatement is a Hono Response object
    ...
    if (typeof generateResponse === 'object' && 'statement' in generateResponse) {
      // Direct object response (shouldn't happen with Hono but handle it)
      statementResponse = generateResponse as unknown as FinancialStatementResponse;
      responseStatus = HttpStatusCodes.OK;
    } else {
      // It's a Response object, we need to clone and parse it
    ...
    // Step 2: Transform into format expected by export functions
    const exportData = {
...
async function exportToPDF(statementData: any, options: any, c: any) {
  ...
  const currentFYStart = statementData.statement.reportingPeriod.year;
  const currentFYEnd = currentFYStart + 1;
  const previousFYStart = currentFYStart - 1;
  const previousFYEnd = currentFYStart;

  const periodEndDate = new Date(statementData.statement.reportingPeriod.endDate);
  ...
  const filename = `${statementData.statement.statementCode}_${statementData.statement.reportingPeriod.year}_${Date.now()}.pdf`;
...
async function exportToExcel(statementData: any, options: any, c: any) {
  ...
  const filename = `${statementData.statement.statementCode}_${statementData.statement.reportingPeriod.year}_${Date.now()}.xlsx`;
...
async function exportToCSV(statementData: any, options: any, c: any) {
  ...
  if (includeMetadata) {
    ...
    csvContent += `Reporting Period,${statementData.statement.reportingPeriod.year}\n`;
  ...
  const filename = `${statementData.statement.statementCode}_${statementData.statement.reportingPeriod.year}_${Date.now()}.csv`;
...
Period: ${statementData.statement.reportingPeriod.year}
