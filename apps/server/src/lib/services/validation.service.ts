...
export class ValidationService {
  ...
  async validateAccountingEquation(
    ...
    try {
      ...
      // Check quarterly balances as well
      const quarters = ['q1', 'q2', 'q3', 'q4'];
      for (const quarter of quarters) {
        const qNetAssets = balances.netFinancialAssets?.[quarter] || 0;
        const qClosingBalance = balances.closingBalance?.[quarter] || 0;
        const qDifference = Math.abs(qNetAssets - qClosingBalance);

        if (qDifference > tolerance) {
          errors.push({
            field: `accounting_equation_${quarter}`,
            message: `${quarter.toUpperCase()} Net Financial Assets (${qNetAssets}) must equal Closing Balance (${qClosingBalance})`,
            code: 'QUARTERLY_IMBALANCE',
            severity: 'error'
          });

          // Log quarterly imbalance
          const quarterlyImbalanceLog = this.createStructuredLog(
            'warn',
            `Quarterly accounting equation imbalance detected: ${quarter}`,
            {
              fieldKey: `accounting_equation_${quarter}`,
              error: `Quarterly difference of ${qDifference.toFixed(2)} exceeds tolerance of ${tolerance}`
            }
          );
          this.logStructured(quarterlyImbalanceLog);
        }
      }
      ...
      return {
        ...
        metadata: {
          ...
          skippedRules: 0,
          processingTime: executionTime,
          totalFields: quarters.length + 1, // quarters + main equation
          validatedFields: quarters.length + 1
        }
      };
