...
export class DataAggregationEngine implements EventDataProcessor {
  ...
  async aggregateByEvent(eventData: EventDataCollection): Promise<EventAggregation> {
    ...
    console.log(`[aggregateByEvent] Starting aggregation with ${eventData.currentPeriod.length} current period entries`);
    ...
    for (const entry of eventData.currentPeriod) {
      ...
      if (eventData.currentPeriod.indexOf(entry) < 3) {
        console.log(`[aggregateByEvent] Entry ${eventData.currentPeriod.indexOf(entry) + 1}: Event ${entry.eventCode}, Amount: ${entry.amount}, Facility: ${entry.facilityId}`);
      }
    ...
    console.log(`[aggregateByEvent] Aggregation complete:`);
    console.log(`  - Total events aggregated: ${eventTotals.size}`);
    console.log(`  - Total facilities: ${facilityTotals.size}`);
    console.log(`  - Total amount: ${totalAmount}`);
    console.log(`  - Sample event totals:`, Array.from(eventTotals.entries()).slice(0, 5));
  ...
  private async collectQuarterlyJsonData(
    ...
    try {
      ...
      for (const jsonEntry of jsonResults) {
        ...
        console.log(`[JSON Structure Detection] Detected structure: ${structureType} for facility ${jsonEntry.facilityId}`);
  ...
  private async processExecutionJsonStructure(
    ...
    if (activities.length === 0) {
      console.warn('[Execution JSON] No activities found in execution structure');
      return eventEntries;
    }

    console.log(`[Execution JSON] Processing ${activities.length} execution activities`);
    ...
    console.log(`[Execution JSON] Generated ${eventEntries.length} entries from execution structure`);
    return eventEntries;
  }
