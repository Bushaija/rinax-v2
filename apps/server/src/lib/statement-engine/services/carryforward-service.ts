...
export class CarryforwardService {
  ...
  private async performCarryforward(options: CarryforwardOptions): Promise<CarryforwardResult> {
    try {
      ...
      if (carryforwardAmount === 0) {
        this.logger.warn('Previous period ending cash is zero');
        return {
          success: true,
          beginningCash: 0,
  ...
  private calculateEndingCashFromExecution(executionData: any): number {
    this.logger.debug('Calculating ending cash from execution data (Section D activities D_1 and D_2 cumulative balances)');
    
    try {
      if (!executionData || !executionData.activities) {
        this.logger.warn('Invalid execution data structure for cash calculation');
        return 0;
      }
      
      const activities = executionData.activities;
      
      // Find Section D activities: D_1 (Cash at bank) and D_2 (Petty cash)
      let cashAtBank = 0;
      let pettyCash = 0;
      ...
      this.logger.debug(
        `Calculated ending cash: Cash at bank (${cashAtBank}) + Petty cash (${pettyCash}) = ${totalCash}`
      );
  ...
  private async getAggregatedBeginningCashWithMetadata(
    ...
    try {
      ...
      this.logger.info(
        ...
        `Missing data for ${facilitiesWithMissingData.length} facilities.`
      );
