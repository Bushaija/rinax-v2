 ...
 * based on user roles (Admin, Accountant) and their facility assignments.
 */
...
export function getDashboardAccessRights(
  ...
  // Accountant: Limited to their district
  if (role === "accountant") {
    ...
    return {
      ...
      canViewBudgetByFacilityChart: true,
      canFilterByAnyProvince: false,
      canFilterByAnyDistrict: false, // Can only view their own district
      canFilterByProgram: true,
      canFilterByQuarter: true,
    };
  }

  // Program Manager: Similar to accountant but might have broader access
...
/**
 * Get default district ID for accountants
 */
