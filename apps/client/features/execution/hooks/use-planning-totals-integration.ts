...
export function usePlanningTotalsIntegration({
  ...
  const planningTotals = useMemo((): PlanningQuarterlyTotals => {
    console.log('🐛 Planning Data Debug:', {
      ...
      enabled,
      planningData,
      activitiesCount: planningData?.activities?.length || 0,
      firstActivity: planningData?.activities?.[0],
      rawPlanningData: planningData
    });
    ...
    console.log('🐛 Calculated Planning Totals:', totals);
