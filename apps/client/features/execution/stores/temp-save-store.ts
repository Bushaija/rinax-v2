  ...
  persist(
    (set, get) => ({
      ...
      saveTemporary: (id, formData, formValues, expandedRows, metadata) => {
        try {
          ...
          console.log({ tag: 'tempSave:saveTemporary', id, size: JSON.stringify({ formData, formValues }).length, hasExisting: !!existingSave })
          
          // Debug: Check Section D items in formData
          console.log('🔍 [tempSave] Checking formData for Section D items...');
          console.log('🔍 [tempSave] formData type:', typeof formData, 'isArray:', Array.isArray(formData));
          console.log('🔍 [tempSave] formData length/keys:', Array.isArray(formData) ? formData.length : Object.keys(formData || {}).length);
          
          if (Array.isArray(formData)) {
            console.log('🔍 [tempSave] formData row IDs:', formData.map((row: any) => row.id));
            const sectionD = formData.find((row: any) => row.id === 'D');
            if (sectionD) {
              console.log('🔍 [tempSave] Section D found:', {
                id: sectionD.id,
                ...
                q3: sectionD.q3,
                q4: sectionD.q4,
                cumulativeBalance: sectionD.cumulativeBalance,
                childrenCount: sectionD.children?.length
              });
              
              if (sectionD.children) {
                sectionD.children.forEach((child: any) => {
                  console.log('🔍 [tempSave] Section D child:', {
                    id: child.id,
                    title: child.title,
                    q1: child.q1,
                    q2: child.q2,
                    q3: child.q3,
                    q4: child.q4,
                    cumulativeBalance: child.cumulativeBalance
                  });
                });
              }
            } else {
              console.log('🔍 [tempSave] Section D NOT FOUND in formData');
            }
          } else {
            console.log('🔍 [tempSave] formData is not an array, structure:', formData);
          }
          ...
          console.log({ tag: 'tempSave:saved', id, lastSaved: now })
        } catch (error: any) {
          ...
          if (error?.name === 'QuotaExceededError' || error?.message?.includes('quota')) {
            ...
            if (cleaned > 0) {
              ...
              try {
                ...
                console.log({ tag: 'tempSave:savedAfterCleanup', id })
              } catch (retryError) {
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        console.log({ tag: 'tempSave:onRehydrateStorage', hasState: !!state })
        if (state) {
