...
export const mapPlanningPayload = (
  ...
  plan.activities.forEach((activity: AnyActivity) => {
    ...
    if (!planningActivityId || !hasCounts) {
      console.log(`🐛 DEBUG: Skipping activity - ${activity.typeOfActivity} - planningActivityId: ${planningActivityId}, hasCounts: ${hasCounts}`);
      return;
    }

    const duplicateKey = `${opts.reportingPeriodId}-${opts.facilityId}-${planningActivityId}`;
    if (seen.has(duplicateKey)) {
      console.log(`🐛 DEBUG: Skipping duplicate activity: ${activity.typeOfActivity}`);
      return;
    }
