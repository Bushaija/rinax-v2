...
export const captureFormState = (
  ...
  console.log('📸 [captureFormState] Starting capture...');
  console.log('📸 [captureFormState] formData length:', formData?.length);
  console.log('📸 [captureFormState] formData sample:', formData?.slice(0, 2).map(r => ({ id: r.id, title: r.title })));
  
  try {
    // Performance optimization: Check data size before cloning
    const dataSize = JSON.stringify(formData).length
    console.log('📸 [captureFormState] Data size:', dataSize, 'bytes');
    ...
    console.log('📸 [captureFormState] Form values keys:', Object.keys(formValues || {}));
    
    // Convert Set to Array for serialization
    const expandedRowsArray = Array.from(expandedRows)
    console.log('📸 [captureFormState] Expanded rows count:', expandedRowsArray.length);
    ...
    console.log('📸 [captureFormState] Cloned data length:', clonedData.length);
...
export const restoreFormState = (
  ...
  try {
    ...
    console.log('Form state restored successfully', {
      formDataRows: savedData.formData.length,
      expandedRows: savedData.expandedRows?.length || 0,
      formValues: Object.keys(savedData.formValues || {}).length
    })
